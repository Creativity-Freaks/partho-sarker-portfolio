...
export function Hero({ speaking }: { speaking: string }) {
  return (
    <section id="top" className="relative min-h-screen flex items-center justify-center px-6 pt-24 pb-16 overflow-hidden">
      {/* Animated DNA orbits */}
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full border border-accent/10 animate-dna-spin" style={{ animationDirection: "reverse", animationDuration: "30s" }} />
        <div className="absolute top-20 left-10 w-2 h-2 rounded-full bg-primary animate-float-slow" />
        <div className="absolute bottom-32 right-20 w-3 h-3 rounded-full bg-accent animate-float-slow" style={{ animationDelay: "2s" }} />
        <div className="absolute top-40 right-1/4 w-1.5 h-1.5 rounded-full bg-gold animate-float-slow" style={{ animationDelay: "4s" }} />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto grid lg:grid-cols-[1.2fr_1fr] gap-12 items-center">
        <div>
          ...
          >
            Bioinformatics researcher decoding molecular structures and
            engineering data-driven solutions for <span className="text-foreground">agricultural innovation</span>.
          </motion.p>
          ...
          >
            <a href="#research" className="group inline-flex items-center gap-2 px-6 py-3 rounded-full bg-primary text-primary-foreground font-medium text-sm hover:glow transition-all">
              <Dna className="h-4 w-4" />
              Explore my research
              <ArrowDown className="h-4 w-4 group-hover:translate-y-0.5 transition-transform" />
            </a>
            <a href="#contact" className="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-border hover:border-primary/50 hover:bg-white/5 font-medium text-sm transition-all">
          ...
          >
            {[
              ...
              <div key={s.l}>
                <div className="text-2xl md:text-3xl font-display font-bold text-gradient">{s.n}</div>
